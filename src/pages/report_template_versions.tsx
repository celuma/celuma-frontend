...
import type { ReportTemplateVersionSummary } from "../models/report";
...
function VersionStatusChip({ status }: { status: ReportTemplateVersionSummary["status"] }) {
    const cfg =
        status === "ACTIVE"
...
interface ReportTemplateVersionsProps {
    embedded?: boolean;
}

function ReportTemplateVersions({ embedded = false }: ReportTemplateVersionsProps) {
    ...
    const load = async () => {
        ...
        setLoading(true);
        try {
            const [template, versionsResp] = await Promise.all([
                getReportTemplateById(templateId),
                listReportTemplateVersions(templateId),
            ]);
            setTemplateName(template.name);
            setVersions(versionsResp.versions);
        } catch (err) {
            message.error(err instanceof Error ? err.message : "Error al cargar las versiones de la plantilla");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleActivate = async (version: ReportTemplateVersionSummary) => {
        if (!templateId) return;
        setBusyVersionId(version.id);
        try {
            await activateReportTemplateVersion(templateId, version.id);
    ...
    const handleArchive = async (version: ReportTemplateVersionSummary) => {
        if (!templateId) return;
        setBusyVersionId(version.id);
        try {
            await archiveReportTemplateVersion(templateId, version.id);
    ...
    const columns: ColumnsType<ReportTemplateVersionSummary> = [
        {
            title: "Versión",
        ...
        },
        {
            ...
            render: (status: ReportTemplateVersionSummary["status"]) => <VersionStatusChip status={status} />,
        },
        {
            title: "Publicada",
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <Tooltip title="Ver esta configuración y publicar una nueva versión a partir de ella">
                        <CelumaButton
                            size="xsmall"
                            disabled={!canManage}
                            onClick={() => navigate(`/config/report-templates/${templateId}/versions/new?from=${version.id}`)}
                        >
                    ...
                    {version.status !== "ACTIVE" && (
                        <Popconfirm
                            ...
                            description={
                                <span>
                                    Esta versión se utilizará como opción predeterminada para reportes
                                    futuros. Los reportes existentes no cambiarán.
                                </span>
                            }
                            okText="Activar"
                            cancelText="Cancelar"
    ...
    const content = (
        <div style={{ display: "grid", gap: tokens.gap }}>
            <PageHeader
                title={templateName ? `Versiones — ${templateName}` : "Versiones de la plantilla"}
                subtitle="Publica, activa y archiva configuraciones de presentación para esta plantilla."
                extra={
                    <div style={{ display: "flex", gap: 8 }}>
                        <CelumaButton onClick={() => navigate("/config/report-templates")}>Volver</CelumaButton>
                        <CelumaButton
                            type="primary"
                            disabled={!canManage}
                            onClick={() => navigate(`/config/report-templates/${templateId}/versions/new`)}
                        >
                            Nueva versión
                        </CelumaButton>
                    </div>
            ...
            <Card style={cardStyle}>
                <CelumaTable
                    ...
                    emptyText="Esta plantilla aún no tiene versiones publicadas"
                    scroll={{ x: 960 }}
                />
            </Card>
        </div>
...
export default ReportTemplateVersions;
